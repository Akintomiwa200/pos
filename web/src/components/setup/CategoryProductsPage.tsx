...
export function CategoryProductsPage({ slug }: { slug: string }) {
  ...
  const displayName = useMemo(() => {
    ...
    const fromProduct = matching[0]?.category;
    if (fromProduct) return fromProduct;
    return slug
      .split("-")
      .map((word) => word.charAt(0).toUpperCase() + word.slice(1))
      .join(" ");
  }, [categories, matching, slug]);

  const filtered = matching.filter((row) => {
    const query = search.trim().toLowerCase();
    if (!query) return true;
    return [row.name, row.sku, row.productCode, row.barcode, row.subcategory, row.brand]
      .filter(Boolean)
  ...
  async function save() {
    ...
    const categoryValue = draft.category.trim() || displayName;
    if (!categoryValue) {
    ...
    try {
      ...
      setFormOpen(false);
      toast.success(draft.id ? "Product updated." : "Product added to this category.");
    } catch (err) {
    ...
    <div className="relative space-y-5 text-pos-ink">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div className="min-w-0">
          ...
          {note ? <p className="mt-1 text-[13px] text-pos-ink-faint">{note}</p> : null}
        </div>
          ...
          <button
            ...
            onClick={() => {
              if (matching.length === 0) {
                toast.info("No products in this category to export.");
                return;
              }
              downloadCsv(`category-${slug}.csv`, matching);
              toast.success("Category products exported.");
            }}
          >
      ...
      <section className="overflow-hidden rounded-[20px] bg-pos-surface shadow-pos-md">
        ...
        {matching.length === 0 ? (
          <div className="flex flex-col items-center justify-center gap-3 py-16 text-center">
            <p className="text-sm text-pos-ink-muted">No products in this category yet.</p>
