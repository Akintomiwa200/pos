...
export function AuthCard({
  ...
  return (
    <main className="flex items-center justify-center px-4 py-16 sm:px-6">
      <div className="w-full max-w-md rounded-2xl bg-pos-surface p-8 shadow-pos-md">
        <div className="mb-8 flex items-center justify-between">
          <BrandLogo />
          <p
            className={`text-xs font-medium ${
              live === null ? "text-pos-ink-faint" : live ? "text-emerald-600" : "text-red-600"
            }`}
          >
        ...
        <p className="mt-1 text-sm text-pos-ink-muted">{copy}</p>
        {children}
      </div>
    </main>
  );
  ...
  return (
    <label className="mt-4 block text-sm font-medium first:mt-6">
      ...
      <input
        ...
        minLength={minLength}
        className="mt-1 w-full rounded-xl border border-pos-border px-3 py-2 font-normal outline-none focus:border-pos-primary"
      />
    </label>
  );
}
...
export function AuthSubmit({ busy, label }: { busy: boolean; label: string }) {
  return (
    <button
      type="submit"
      disabled={busy}
      className="mt-6 w-full rounded-xl bg-pos-primary py-3 font-semibold text-white disabled:opacity-60"
    >
...
export function AuthLinks({
  ...
  return (
    <p className="mt-5 flex flex-wrap justify-center gap-x-4 gap-y-1 text-center text-sm text-pos-ink-muted">
      {items.map((item) => (
        <Link key={item.href} href={item.href} className="font-medium text-pos-primary">
          {item.label}
        </Link>
      ))}
    </p>
